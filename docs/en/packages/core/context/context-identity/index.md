...
`ContextIdentity<T>` is a portable immutable identity for a typed value resolved through the active ownership chain.

## Definition

The public [`Context.create()` API](../../api/index.md) creates an identity either without a default or with one immutable default value. Creating an identity does not require a `ReactiveRuntime`, allowing libraries to export contexts as package-level objects.
...
`context.provide(scope, value)` registers one value for that identity on a scope. Registration must occur before the scope's first `run()` and may occur only once per identity and scope. Duplicate or late registration throws.
...
`context.get()` starts from the active owner and walks toward the runtime root. The nearest matching scope provider wins. If no provider exists, the context default is returned. An identity without a provider or default throws a missing-context error.
